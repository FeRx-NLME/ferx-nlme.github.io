---
title: "Fit options"
description-meta: "Configure ferx estimation methods, optimizers, convergence controls, covariance, diagnostics, simulation, and advanced NLME fit settings."
open-graph:
  title: "Fit options"
  description: "Configure ferx estimation methods, optimizers, convergence controls, covariance, diagnostics, simulation, and advanced NLME fit settings."
twitter-card:
  title: "Fit options"
  description: "Configure ferx estimation methods, optimizers, convergence controls, covariance, diagnostics, simulation, and advanced NLME fit settings."
---
...
- Check `ferx_get_warnings(fit)` — boundary parameters, high condition number,
...
- Plot the trace (`optimizer_trace = TRUE` then `plot(fit)`) —
...
- Switch from a gradient-based optimizer (`slsqp`, `nlopt_lbfgs` — which is
  what `auto` picks when an analytic gradient exists) to derivative-free
  (`bobyqa`) — the
...
|---|---|---|
| `auto` | Per-model selection **(default)** | Resolves to `nlopt_lbfgs` when the exact analytic FOCE/FOCEI gradient is available, and to `bobyqa` when the outer loop must use finite differences (a model outside the analytic scope, or `gradient = fd`). Two overrides: more than 64 free parameters → `nlopt_lbfgs` even on an FD gradient; mixture models → `bobyqa` regardless. A FOCE/FOCEI fit records the pick as `auto (<resolved>)` (`gn_hybrid` reports `gn`; mixture labels are currently wrong, ferx-core#1540). |
| `bobyqa` | NLopt BOBYQA — derivative-free | Robust to small surface discontinuities; what `auto` picks without an analytic gradient. |
...
| `nlopt_lbfgs` | NLopt L-BFGS (`lbfgs` / `bfgs` are deprecated aliases) | Moderate-sized parameter spaces; what `auto` picks with an analytic gradient. |
...
The legacy `ad` value is no longer accepted and returns an `E_AD_RETIRED`
error — use `auto` or `fd` instead.
...
|---|---|---|
| `optimizer_trace` | `false` | Write per-iteration CSV to a temp file. Read with `ferx_trace(fit)`; plot with `plot(fit)`. |
