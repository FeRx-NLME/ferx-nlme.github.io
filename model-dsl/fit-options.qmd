...
|---|---|---|
| `auto` | Per-model selection **(default)** | Resolves to `nlopt_lbfgs` when the exact analytic FOCE/FOCEI gradient is available, and to `bobyqa` when the outer loop must use finite differences (ODE/PD models, LTBS, SDE, `gradient_method = fd`, mixture models). The fit output records the pick as `auto (<resolved>)`. |
