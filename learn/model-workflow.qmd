...
description-meta: "Inspect, edit, fit, and diagnose population PK/PD models in R using path-based and pipe-based ferx workflows."
open-graph:
  title: "Model workflow: inspect, edit, fit"
  description: "Inspect, edit, fit, and diagnose population PK/PD models in R using path-based and pipe-based ferx workflows."
twitter-card:
  title: "Model workflow: inspect, edit, fit"
  description: "Inspect, edit, fit, and diagnose population PK/PD models in R using path-based and pipe-based ferx workflows."
---
...
`ferx_model_get_section()` prints the named section to the console and passes the
`ferx_model` object through unchanged, so the pipe continues:

```r
ferx_model(ex$data, ex$model) |>
  ferx_model_get_section("parameters")
```
...
`ferx_model_set_section()` rewrites a section on disk and returns the `ferx_model`
...
functions (`fit$estimates`, `fit$cor_matrix`, `plot()`,
...
ferx_model(ex$data, ex$model) |>
  ferx_model_get_section("parameters") |>  # optional: peek before fitting
  ferx_fit(method = "focei", covariance = TRUE) |>
  summary()

# Edit options then fit. ferx_model_set_section() auto-copies bundled examples to
