...
description-meta: "Evaluate ferx NLME fits with parameter uncertainty, goodness-of-fit plots, residuals, shrinkage, correlations, and convergence diagnostics."
open-graph:
  title: "Post-fit diagnostics"
  description: "Evaluate ferx NLME fits with parameter uncertainty, goodness-of-fit plots, residuals, shrinkage, correlations, and convergence diagnostics."
twitter-card:
  title: "Post-fit diagnostics"
  description: "Evaluate ferx NLME fits with parameter uncertainty, goodness-of-fit plots, residuals, shrinkage, correlations, and convergence diagnostics."
---
...
## Parameter estimates — `fit$estimates`

`fit$estimates` collects all estimated parameters (theta, omega, sigma) into
...
## Parameter correlation matrix — `fit$cor_matrix`

`fit$cor_matrix` converts the estimated parameter covariance matrix to
...
## ETA-covariate correlations — `fit$eta_cov`

`fit$eta_cov` computes Pearson correlations between individual ETA estimates
...
## Structured warnings — `ferx_get_warnings()`

ferx classifies non-fatal issues from a run into severity levels and categories
and makes them available via `ferx_get_warnings()`. Critical issues are also surfaced
...
## Optimizer trace — `ferx_trace()` / `plot()`
...
plot(fit)           # two-panel plot: OFV + method-specific metric
plot(fit, log_ofv = TRUE)  # OFV on log scale (OFV - min(OFV))
```
